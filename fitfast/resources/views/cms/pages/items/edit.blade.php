...
<div class="row">
    <div class="col-12">
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('cms.items.update', $item) }}" method="POST" id="itemForm" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
...
function updateImageOrder() {
    ...
    fetch(`/cms/items/{{ $item->id }}/images/reorder`, {
        method: 'POST',
        headers: {
    ...
    if (e.target.classList.contains('remove-existing-image') || e.target.closest('.remove-existing-image')) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`/cms/items/${itemId}/images/${imageId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json',
...
document.addEventListener('click', function(e) {
    if (e.target.classList.contains('set-primary-image') || e.target.closest('.set-primary-image')) {
        ...
        fetch(`/cms/items/${itemId}/images/${imageId}/set-primary`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
